...
class mysql {
	...
	function select($options) {
		...
		$sql = "SELECT {$options['fields']} FROM {$options['table']} WHERE {$options['condition']} ORDER BY {$options['order']} {$options['limit']}";
	...
	function update($table=null,$array_of_values=array(),$conditions='FALSE') {
        ...
        //if($debug = 1) echo "UPDATE $table SET $what_to_set_string WHERE $conditions";
	...
	function insert($table=null,$array_of_values=array()) {
		...
		//print_r($s);
		if ($this->execute($s)) return mysql_insert_id($this->con); // mysql_insert_id($this->con);
		return false;
	}
